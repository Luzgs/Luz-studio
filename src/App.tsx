...
const INVENTORY = [
  {
    id: 's1',
    category: 'Cejas',
    name: 'Cejas Depiladas y Pigmentado',
    priceEur: 5,
  },
...
const LogoLG = () => (
  <svg
    viewBox="0 0 300 160"
    className="w-full h-32 max-w-[250px] mx-auto drop-shadow-md"
  >
    <defs>
...
const App = () => {
  ...
  useEffect(() => {
    const fetchRate = async () => {
      try {
        ...
        console.error(
          'No se pudo obtener la tasa, usando tasa de emergencia:',
          error
        );
      } finally {
  ...
  // --- CÁLCULOS ---
  const subtotalEur = selectedServices.reduce(
    (sum, service) => sum + service.priceEur,
    0
  );
  const hasDiscount = selectedServices.length >= 2;
  const discountAmountEur = hasDiscount ? subtotalEur * DISCOUNT_PERCENTAGE : 0;
  const domicilioFeeEur =
    formData.modality === 'domicilio' ? DOMICILIO_FEE_EUR : 0;
  ...
  const handleWhatsAppCheckout = () => {
    ...
    message += `📍 *Modalidad:* ${
      formData.modality === 'estudio' ? 'En el Estudio' : 'A Domicilio'
    }\n`;

    if (formData.modality === 'estudio') {
      message += `🏢 *Sede elegida:* ${
        formData.selectedStudio === 'bebedero'
          ? 'Sede Bebedero'
          : 'Sede Campeche'
      }\n`;
    ...
    message += `Subtotal: €${subtotalEur.toFixed(2)} / Bs.${eurToBs(
      subtotalEur
    )}\n`;
    if (hasDiscount) {
      message += `🎁 Descuento Promo (10%): -€${discountAmountEur.toFixed(
        2
      )} / -Bs.${eurToBs(discountAmountEur)}\n`;
    }
    if (formData.modality === 'domicilio') {
      message += `🚗 Costo Domicilio: +€${domicilioFeeEur.toFixed(
        2
      )} / +Bs.${eurToBs(domicilioFeeEur)}\n`;
    }
    message += `\n*TOTAL A PAGAR: €${totalEur.toFixed(2)} / Bs.${eurToBs(
      totalEur
    )}*\n\n`;
      ...
      <main className="max-w-2xl mx-auto px-4 mt-8 space-y-6">
        ...
        <div className="bg-[#fffdf5] border border-[#ebd8b0] rounded-xl p-3 flex items-center justify-center text-sm shadow-sm">
          {isRateLoading ? (
            ...
            <span className="text-[#8a6519] font-medium flex items-center">
              ...
              Tasa del día (BCV):{' '}
              <strong className="ml-1 text-lg"> {euroRate} Bs/€</strong>
            </span>
          )}
        </div>
        ...
        <section className="bg-white rounded-2xl p-6 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.1)] border border-[#f5ead5]">
          <h2 className="text-xl font-bold text-gray-800 mb-5 flex items-center">
            <span className="bg-gradient-to-r from-[#BF953F] to-[#B38728] text-white rounded-full w-7 h-7 flex items-center justify-center mr-3 text-sm font-bold shadow-md">
              1
            </span>
            Elige tus Servicios
          </h2>
              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-between p-4 rounded-xl cursor-pointer transition-all duration-300 border-2 ${
                    isSelected
                      ? 'border-[#B38728] bg-[#fffdf5] shadow-md transform scale-[1.01]'
                      : 'border-gray-100 hover:border-[#ebd8b0] bg-white'
                  }`}
                >
                  <div className="flex items-center space-x-4">
                    <div
                      className={`w-6 h-6 rounded-md flex items-center justify-center border transition-colors ${
                        isSelected
                          ? 'bg-gradient-to-br from-[#BF953F] to-[#AA771C] border-transparent'
                    ...
                    <div>
                      <p
                        className={`font-semibold text-sm sm:text-base ${
                          isSelected ? 'text-[#8a6519]' : 'text-gray-700'
                        }`}
                      >
                        {service.name}
                      </p>
                  ...
                  <div className="text-right">
                    <p
                      className={`font-bold text-lg ${
                        isSelected ? 'text-[#B38728]' : 'text-gray-600'
                      }`}
                    >
          ...
          {showGallery && (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-5">
              {GALLERY_IMAGES.map((src, index) => (
                <div
                  key={index}
                  className="rounded-xl overflow-hidden shadow-sm border border-gray-100 aspect-square bg-gray-50 flex items-center justify-center"
                >
                  <img
                    src={src}
                    alt={`Resultado de trabajo ${index + 1}`}
            ...
            <div className="pt-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  type="button"
                  onClick={() =>
                    setFormData({ ...formData, modality: 'estudio' })
                  }
                  className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all ${
                    formData.modality === 'estudio'
                      ? 'border-[#B38728] bg-[#fffdf5] text-[#8a6519] shadow-sm'
                ...
                <button
                  type="button"
                  onClick={() =>
                    setFormData({ ...formData, modality: 'domicilio' })
                  }
                  className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all ${
            ...
            {formData.modality === 'estudio' ? (
                ...
                <div className="space-y-2">
                  <label
                    className={`flex items-start p-3 rounded-lg border cursor-pointer transition-colors ${
                      formData.selectedStudio === 'bebedero'
                        ? 'bg-white border-[#B38728] shadow-sm'
                  ...
                  >
                    ...
                    <span className="ml-2 text-sm text-gray-700">
                      <strong>Sede Bebedero:</strong> <br />
                      {DIRECCIONES_ESTUDIO.bebedero}
                    </span>
                  </label>
                  <label
                    className={`flex items-start p-3 rounded-lg border cursor-pointer transition-colors ${
                      formData.selectedStudio === 'campeche'
                        ? 'bg-white border-[#B38728] shadow-sm'
                        : 'border-transparent hover:bg-white/50'
                    }`}
                  >
                    ...
                    />
                    <span className="ml-2 text-sm text-gray-700">
                      <strong>Sede Campeche:</strong> <br />
                      {DIRECCIONES_ESTUDIO.campeche}
                    </span>
                  </label>
                </div>
              </div>
              ...
              <div className="mt-2 space-y-3">
                <div className="bg-amber-50 p-3 rounded-lg text-sm text-amber-800 flex items-center border border-amber-200">
                  ...
                  <span>
                    Se aplica un costo adicional de{' '}
                    <strong>
                      €{DOMICILIO_FEE_EUR}{' '}
                      {!isRateLoading && `/ Bs.${eurToBs(DOMICILIO_FEE_EUR)}`}
                    </strong>{' '}
                    por traslado.
                  </span>
                </div>
          ...
          <div className="border-t border-gray-200 pt-5 space-y-3">
            <div className="flex justify-between text-gray-600">
              ...
              <span>
                ...
                {!isRateLoading && `/ Bs.${eurToBs(subtotalEur)}`}
              </span>
            </div>
            {hasDiscount && (
              <div className="flex justify-between text-green-600 font-medium bg-green-50 px-2 py-1 rounded">
                ...
                <span>
                  -€{discountAmountEur.toFixed(2)}{' '}
                  {!isRateLoading && `/ -Bs.${eurToBs(discountAmountEur)}`}
                </span>
              </div>
            )}
            {formData.modality === 'domicilio' && (
              <div className="flex justify-between text-[#B38728] font-medium bg-[#fffdf5] px-2 py-1 rounded">
                ...
                <span>
                  +€{domicilioFeeEur.toFixed(2)}{' '}
                  {!isRateLoading && `/ +Bs.${eurToBs(domicilioFeeEur)}`}
                </span>
              </div>
            )}
            <div className="flex justify-between text-xl font-black text-gray-900 pt-3 border-t border-gray-200 mt-2">
              ...
              <div className="text-right">
                <span className="block text-2xl text-[#B38728]">
                  €{totalEur.toFixed(2)}
                </span>
                {!isRateLoading && (
